...
async function detailPage() {
  ...
  try {
    ...
    document.querySelector("#detail").innerHTML = `
      ...
      <section class="section card"><div class="card-body"><h2>주소 / 교통</h2><p class="muted">${escapeHtml(hotel.address || "-")}</p>${trans.length ? trans.map((item) => `<span class="pill">${escapeHtml(item.name)} ${escapeHtml(item.time || "")} ${escapeHtml(item.depart || "")}</span>`).join(" ") : empty("등록된 교통 정보가 없습니다.")}</div></section>
