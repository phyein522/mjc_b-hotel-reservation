...
public class TossPaymentRestController {
    ...
    public ResponseEntity<ApiResponse<PaymentDto>> insert(@RequestBody PaymentDto dto) {
        PaymentDto insert = (PaymentDto) new PaymentDto().copyMembers(tossPaymentService.insertPayment(dto), true);
...
//    }
    ...
    public ResponseEntity<ApiResponse<PaymentDto>> confirmTossPayment(@RequestBody TossPaymentConfirmRequestDto dto) {
        PaymentDto result = tossPaymentService.confirmTossPayment(dto);
    ...
    public ResponseEntity<ApiResponse<PaymentDto>> failTossPayment(@RequestBody TossPaymentFailRequestDto dto) {
        PaymentDto result = tossPaymentService.failTossPayment(dto);
        return ResponseEntity.ok(
    ...
    public ResponseEntity<ApiResponse<List<PaymentDto>>> getPayments() {
        List<PaymentDto> payments = tossPaymentService.getPayments().stream()
    ...
    public ResponseEntity<ApiResponse<PaymentDto>> getPayment(@PathVariable Long paymentId) {
        PaymentDto result = tossPaymentService.getPayment(paymentId);
        return ResponseEntity.ok(
    ...
    public ResponseEntity<ApiResponse<PaymentDto>> update(
        ...
        PaymentDto result = tossPaymentService.updatePayment(dto.getPaymentId(), dto);
        return ResponseEntity.ok(
    ...
    public ResponseEntity<ApiResponse<PaymentDto>> delete(@PathVariable Long paymentId) {
        PaymentDto result = tossPaymentService.deletePayment(paymentId);
        return ResponseEntity.ok(
                new ApiResponse<>(ResponseCode.SUCCESS, "payments delete success", result)
        );
    }
}
